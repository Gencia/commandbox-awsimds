...
Within any of the places where you can use system setting expansion (e.g server.json) reference the IMDS variable you wish to retrieve by prefixing the variable name with `awsimds` - e.g. to access the instanceId of the host AWS EC2 instance, use `{awsimds.instance-id}`
